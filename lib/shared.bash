...
function plugin_read_config() {
  local var="BUILDKITE_PLUGIN_PODMAN_COMPOSE_${1}"
  local default="${2:-}"
  echo "${!var:-$default}"
}
...
function plugin_read_list() {
  prefix_read_list "BUILDKITE_PLUGIN_PODMAN_COMPOSE_$1"
}
...
# Returns the name of the podman compose project for this build
function podman_compose_project_name() {
  # No dashes or underscores because podman-compose will remove them anyways
  echo "buildkite${BUILDKITE_JOB_ID//-}"
}

# Runs podman ps -a filtered by the current project name
function podman_ps_by_project() {
  podman ps -a \
    --filter "label=com.podman.compose.project=$(podman_compose_project_name)" \
    "${@}"
}
...
# Runs the podman-compose command, scoped to the project, with the given arguments
function run_podman_compose() {
  local command=(podman-compose)
  ...
  command+=(-p "$(podman_compose_project_name)")
