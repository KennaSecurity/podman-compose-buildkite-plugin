...
# and then runs podman-compose run a generated project name

run_service="$(plugin_read_config RUN)"
container_name="$(podman_compose_project_name)_${run_service}_build_${BUILDKITE_BUILD_NUMBER}"
...
for service_name in "${prebuilt_candidates[@]}" ; do
  if prebuilt_image=$(get_prebuilt_image "$service_name") ; then
    echo "~~~ :podman: Found a pre-built image for $service_name"
...
if [[ ${#pull_services[@]} -gt 0 ]] ; then
  echo "~~~ :podman: Pulling services ${pull_services[0]}"
  retry "$pull_retries" run_podman_compose "${pull_params[@]}"
...
# Parse BUILDKITE_PODMAN_DEFAULT_VOLUMES delimited by semi-colons, normalized to
# ignore spaces and leading or trailing semi-colons
IFS=';' read -r -a default_volumes <<< "${BUILDKITE_PODMAN_DEFAULT_VOLUMES:-}"
...
if [[ "${BUILDKITE_PLUGIN_PODMAN_COMPOSE_REQUIRE_PREBUILD:-}" =~ ^(true|on|1)$ ]] && [[ ! -f "$override_file" ]] ; then
  ...
  echo "~~~ :podman: Building Podman Compose Service: $run_service" >&2
...
if [[ "${BUILDKITE_PLUGIN_PODMAN_COMPOSE_SHELL:-}" =~ ^(false|off|0)$ ]] ; then
  shell_disabled=1

# Show a helpful error message if a string version of shell is used
elif [[ -n "${BUILDKITE_PLUGIN_PODMAN_COMPOSE_SHELL:-}" ]] ; then
  echo -n "🚨 The Podman Compose Plugin’s shell configuration option must be specified as an array. "
...
elif plugin_read_list_into_result BUILDKITE_PLUGIN_PODMAN_COMPOSE_SHELL ; then
  shell_disabled=''
...
if [[ -n "${BUILDKITE_PLUGIN_PODMAN_COMPOSE_COMMAND:-}" ]] ; then
  echo "🚨 The Podman Compose Plugin’s command configuration option must be an array."
  exit 1
fi

# Parse plugin command if provided
if plugin_read_list_into_result BUILDKITE_PLUGIN_PODMAN_COMPOSE_COMMAND ; then
  for arg in "${result[@]}" ; do
    command+=("$arg")
  done
fi
...
(
  echo "+++ :podman: Running ${display_command[*]:-} in service $run_service" >&2
  run_podman_compose "${run_params[@]}"
)
...
if [[ -n "${BUILDKITE_AGENT_ACCESS_TOKEN:-}" ]] ; then
  if [[ "$(plugin_read_config CHECK_LINKED_CONTAINERS "true")" != "false" ]] ; then
    ...
    done <<< "$(podman_ps_by_project -q)"

    failed_containers=()
    if [[ 0 != "${#containers[@]}" ]] ; then
      while read -r container ; do
        [[ -n "$container" ]] && failed_containers+=("$container")
      done <<< "$(podman inspect -f '{{if ne 0 .State.ExitCode}}{{.Name}}.{{.State.ExitCode}}{{ end }}' \
        "${containers[@]}")"
    fi

    if [[ 0 != "${#failed_containers[@]}" ]] ; then
      echo "+++ :warning: Some containers had non-zero exit codes"
      podman_ps_by_project \
        --format 'table {{.Label "com.podman.compose.service"}}\t{{ .ID }}\t{{ .Status }}'
    fi
