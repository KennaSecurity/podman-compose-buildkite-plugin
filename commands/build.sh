...
if [[ -z "$image_repository" ]] ; then
  ...
  echo "This build step has no image-repository set. Without an image-repository, the image won't be pushed to a repository, and won't be automatically used by any run steps."
fi
...
for line in $(plugin_read_list CACHE_FROM) ; do
  ...
  echo "~~~ :podman: Pulling cache image for $service_name"
  if retry "$pull_retries" plugin_prompt_and_run podman pull "$service_image" ; then
    printf -v "$cache_image_name" "%s" "$service_image"
  else
    echo "!!! :podman: Pull failed. $service_image will not be used as a cache for $service_name"
  fi
done
